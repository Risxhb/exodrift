...
- Flak is an automatic area-denial screen inspired by a “holy wall of flak.” It begins online, selects an identified hostile track, leads its movement, and repeatedly sends staggered seven-round curtains into a 250 m saturation sector between the carrier and the hostile fleet. The IFF director deconflicts allied craft and ordnance; the wall destroys hostile interceptable missiles and nuclear torpedoes, deals full damage to hostile strikecraft, and applies only 25% damage to hostile capital ships. `1` toggles the system, and the flak-director upgrade extends curtain placement from 3.2 km to 4 km.
...
- One player fleet carrier with six retractable side galleries, a dorsal EW-drone hive, automatic IFF-deconflicted flak screening, lock-on missiles, one nuclear torpedo, automated point defense, layered durability, passive sensors, and active ping.
...
- GL-compatible Blender-authored projectile, shield-lattice, shockwave, armor-debris, bidirectional-warp, Raptor, and sky-accent meshes use shared materials and bounded reusable impact slots; missile exhaust, flak tracers, muzzle flashes, nebula cards, parallax dust, and tiered backdrop visibility remain quality-scaled.
- Committing to a combat sector now opens a three-stage carrier travel sequence: the selected carrier enters an authored warp aperture, remains visible in a streamed in-transit tunnel above a bottom-frame loading bar, then exits through a destination-sector aperture before tactical control is handed to the loaded encounter.
...
- The title screen uses a compact bottom horizontal command row over a staged seven-capital, thirteen-craft engagement. The distant hostile fleet warps in, launches strike craft and secondary salvos, Sidebay answers with an automatic flak curtain and its air group, and friendly frigates unleash their own salvos before the battle loop resolves. Secondary panels remain centered and the browser build omits the desktop-only Quit action without leaving a gap.
...
- Automatic flak computes a lead on the best identified hostile track and fires a paced staggered seven-round curtain along that fleet bearing. Every round bursts inside a 250 m IFF-deconflicted sector placed between the forces, up to 3.2 km or 4 km with the flak director. The sector destroys hostile interceptable ordnance, heavily damages hostile strikecraft, and lightly damages hostile capital ships; `1` toggles the system instead of firing each curtain.
...
- The title battle instantiates the exact playable carrier plus current textured capital/fighter builders against the procedural sky, with three hostile warp rifts, four launch flashes, 28 flak tracers, twelve arcing missile plumes, and four layered explosion rigs. The public fleet archive replaces seven CSS glyphs with 900×506 direct Godot renders of the actual Sidebay, Resolute, Raptor, Watcher, Acheron, Vesper, and Crucible models.
...
- Sidebay's base flak curtain envelope is 3.2 km, extended to 4 km by the flak director. Automatically refreshed seven-round curtains use a 250 m falloff volume and pooled WWII-inspired flashes, smoke clusters, fragments, and pressure rings. They retain hostile ordnance interception, IFF fire-lane deconfliction, and reduced-flash behavior.
...
M17 tests cover the remappable automatic-flak toggle plus 2/3 ordnance actions, fixed curtain envelope, staggered IFF-deconflicted screens, hostile airburst interception and role-scaled damage, nuclear inventory/arming/AoE/friendly fire/interception/trails, heavy carrier limits, speed-reactive engine trails, queued wing redeploy, staged runtime-model title choreography, layered menu effects, pooled VFX bounds, and regression compatibility.
...
Carrier-combat integration tests cover unified mouse heading/camera control without hull snap, automatic seven-round fleet screens, four-missile salvos, nuclear safety behavior, pulsing contact radar, layered deep-space backdrop, flight-operation locks and redeploy, emergency sealing, pursuit exposure, and the closed-bay jump interlock.
...
- **2026-07-15:** Promoted the flak wall into an automatic, default-online, IFF-deconflicted fleet screen with `1` as its toggle, moved hostile opening formations deeper into the battlespace, and rebuilt the main-menu battle as a staged warp-in, hostile launch/salvo, friendly flak/launch, and frigate counter-salvo sequence.
- **2026-07-16:** Added the Blender-authored combat-effects library and production Raptor: guided and nuclear bodies, geodesic shield hits, conventional/nuclear shock rings, armor shards and debris, authored title-screen warp-in plus withdrawal warp-off, and tiered ringed-moon/asteroid/nebula sky accents. The focused asset/VFX suite, ship pipeline, readability, surface, title-layout, integration, and 1080p/1440p performance gates pass.
- **2026-07-16:** Added the sector-travel loading sequence between campaign course confirmation and combat: carrier departure, asynchronous warp transit, bottom-frame progress telemetry, destination-palette arrival, and a black-frame tactical handoff that keeps the campaign map hidden until the encounter is ready.
