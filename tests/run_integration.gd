...
func _run() -> void:
	...
	game._on_target_navigation_requested(&"orbit", &"hostile_command", 10000.0)
	_assert_true(carrier.target_navigation_mode == PlayerCarrier.TargetNavigationMode.ORBIT and is_equal_approx(carrier.target_navigation_distance_m, 10000.0), "distance-bearing navigation accepts the 10 km orbit option")
	game._on_target_navigation_requested(&"keep_distance", &"hostile_command", 25000.0)
	_assert_true(carrier.target_navigation_mode == PlayerCarrier.TargetNavigationMode.KEEP_DISTANCE and is_equal_approx(carrier.target_navigation_distance_m, 25000.0), "distance-bearing navigation preserves the requested 25 km keep range")
	game._on_target_navigation_requested(&"clear", &"hostile_command", 0.0)
	_assert_true(carrier.target_navigation_mode == PlayerCarrier.TargetNavigationMode.NONE, "relative navigation can be cleared without resolving a new target")
