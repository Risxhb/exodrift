...
func _draw_contacts(center: Vector2, radius: float) -> void:
	if not is_instance_valid(carrier) or sensors == null:
		return
	...
	for contact_value in sensors.contacts.values():
		...
		var offset := contact.estimated_position - carrier.global_position
		...
		var identified := contact.identification_state == SensorContact.IdentificationState.IDENTIFIED
		...
		if contact.uncertainty_radius_m > 20.0:
			var uncertainty := clampf(contact.uncertainty_radius_m / display_range_m * radius, 4.0, 24.0)
