...
func summary_text() -> String:
	...
	return """EXODRIFT M15 PLAYTEST DEBRIEF
...
COUNTERS
Pings %d | Tactical opens %d | Orders %d | Wing launches %d | Recalls %d
...
NOTES
%s""" % [str(session.get("session_id", "unknown")), str(session.get("platform", "unknown")), str(session.get("run_id", "none")), _yes_no(bool(acceptance.onboarding_completed)), _yes_no(bool(acceptance.used_active_ping)), _yes_no(bool(acceptance.opened_tactical_map)), _yes_no(bool(acceptance.issued_orders)), _yes_no(bool(acceptance.launched_wings)), _yes_no(bool(acceptance.recalled_wings)), int(counters.get("active_pings", 0)), int(counters.get("tactical_opens", 0)), int(counters.get("orders_issued", 0)), int(counters.get("wing_launches", 0)), int(counters.get("wing_recalls", 0)), int(counters.get("flak_barrages", 0)), int(counters.get("missile_salvos", 0)), int(counters.get("withdrawals", 0)), battles.size(), "\n".join(battle_lines) if not battle_lines.is_empty() else "- No completed battles recorded.", str(session.get("tester_notes", ""))]
